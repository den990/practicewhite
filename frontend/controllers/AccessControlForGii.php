    ...
    public function beforeAction($action)
    {
        if (parent::beforeAction($action)) {
            ...
            if (Yii::$app->user->isGuest) {
                throw new ForbiddenHttpException('Access denied. You must be logged in and be admin');
            }

            // Проверяем, является ли пользователь администратором
            $idUser = Yii::$app->user->getId(); // Предполагаем, что у вас есть такой метод в модели User
            if ($idUser)
            {
                $modelUserRole = UserRole::find()->where(['idUser' => $idUser])->one();
            }
            if ($modelUserRole->role != UserRole::ROLE_ADMIN) {
                throw new ForbiddenHttpException('Access denied. You do not have permission to perform this action.');
            }
